...
| Auto-Smelting Upgrade | Smelts incoming supported pickups when a matching seed already exists | All tiers |
| Basic Compression Upgrade | Crafts self 2x2 compression recipes automatically | All tiers |
| Advanced Compression Upgrade | Crafts self 3x3 compression recipes automatically | All tiers |
...
Auto-Feeding automatically consumes stored food when the player needs hunger.

It uses the real stored item instead of generating free hunger, so consumables are actually consumed unless the item itself has special behavior that preserves it.
