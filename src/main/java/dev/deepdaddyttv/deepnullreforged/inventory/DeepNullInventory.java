...
public class DeepNullInventory extends ItemStackHandler {
    ...
    public boolean clearFluidSlot(int slot) {
        validateSlotIndex(slot);
        if (!supportsFluidStorage() || fluidStacks.get(slot).isEmpty()) {
            return false;
        }
        fluidStacks.set(slot, FluidStack.EMPTY);
        save();
    ...
    public boolean hasDeepEnergyUpgrade() {
        return hasEnergyUpgrade() && hasUpgrade(DeepNullUpgradeType.DEEP_ENERGY);
    }
    ...
    private @Nullable SmeltConversion resolveAutoSmelt(ItemStack stack) {
        if (!hasAutoSmeltingUpgrade() || stack.isEmpty()) {
            return null;
        }
    ...
    private final class UpgradeItemHandler extends ItemStackHandler {
        ...
        public boolean isItemValid(int slot, ItemStack stack) {
            ...
            if (type == null || !supportsUpgrade(type) || !stackHasUpgrade(type, stack)) {
                return false;
            }
            return type != DeepNullUpgradeType.DEEP_ENERGY || hasUpgrade(DeepNullUpgradeType.ENERGY);
        }
