...
public final class DeepNullHudRenderer {
    ...
    public static void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (selectedSlot < 0 || (fluidMode ? selectedFluid.isEmpty() : selectedStack.isEmpty())) {
            return;
        }
        ...
        if (fluidMode) {
            nameLines = font.split(selectedFluid.getHoverName(), PANEL_WIDTH - 34);
            appendWrapped(detailLines, font, label("dn.amount.desc", Component.literal(fluidAmountText(inventory, selectedFluid))), PANEL_WIDTH - 34);
    ...
    private static String fluidAmountText(DeepNullInventory inventory, FluidStack selectedFluid) {
        return inventory.tier().creative()
                ? Component.translatable("dn.infinite.desc").getString()
                : selectedFluid.getAmount() + " mB";
    }
