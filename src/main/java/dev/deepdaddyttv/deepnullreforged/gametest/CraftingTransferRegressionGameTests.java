...
public final class CraftingTransferRegressionGameTests implements FabricGameTest {
    ...
    public static void jei_transfer_handles_large_stored_counts_from_carried_deepnull(GameTestHelper helper) {
        ...
        int expectedCraftCount = inventory.getStackInSlot(0).getCount() / 8;
        helper.assertValueEqual(plan.craftCount(), expectedCraftCount, "Max-transfer planning should use the full stored DeepNull count after slot-cap normalization");
        ...
        for (int i = 0; i < context.craftMatrix().getContainerSize(); i++) {
            ...
            if (stack.is(OAK_PLANKS)) {
                plankSlots++;
                helper.assertValueEqual(stack.getCount(), expectedCraftCount, "Each required 3x3 slot should receive the planned large-count transfer amount");
            }
        }
