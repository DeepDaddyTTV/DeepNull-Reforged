...
public class DeepNullMenu extends AbstractContainerMenu {
    ...
    public boolean hasUpgrade(DeepNullUpgradeType type) {
        return (syncedUpgradeMask & (1 << type.slot())) != 0;
    }
    ...
    public boolean setChargingEnabled(boolean chargingEnabled) {
        if (!hasUpgrade(DeepNullUpgradeType.ENERGY)) {
            return false;
        }
    ...
    public int getDisplayedEnergyStored() {
        return hasUpgrade(DeepNullUpgradeType.ENERGY) ? syncedEnergyStored : 0;
    }

    public int getDisplayedEnergyCapacity() {
        return hasUpgrade(DeepNullUpgradeType.ENERGY) ? dankInventory.getEnergyCapacity() : 0;
    }

    public boolean isChargingEnabledDisplayed() {
        return hasUpgrade(DeepNullUpgradeType.ENERGY) && syncedChargingEnabled;
    }
    ...
    private int addUpgradeSlots() {
        ...
        for (DeepNullUpgradeType type : DeepNullUpgradeType.values()) {
            addSlot(new UpgradeSlot(dankInventory, type.slot(), leftPadding + type.slot() * SLOT_SPACING, TOP_PADDING));
        }
    ...
    private boolean usesIntegratedEnergyLayout() {
        return hasUpgrade(DeepNullUpgradeType.ENERGY)
                ...
                && (viewMode == ViewMode.MAIN || viewMode == ViewMode.UPGRADES || viewMode == ViewMode.FILTER);
    }
